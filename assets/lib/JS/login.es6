/*function Con(){
   ...
   var Ps = "KVAFamily";
    ...
    if((g=10) ){function check(){
    var mysql = require('mysql');

    var con_Get = mysql.createConnection({
    host: "localhost",
    user: "root",
    password: "dingdong",
    database: "vjhex"
    ...
    if (err) throw err;
    con_Get.query("SELECT * FROM customers", function (err, result, fields) {
        if (err) throw err;
        console.log(result);
    });
    });
...
form.addEventListener('submit',(e) =>{
    ...
    if(errors.length >0){
        e.preventDefault()var allInput =[un_input , ps_input ].filter(input => input != null)
